...
def clusterAndContainerInstanceByInstanceId(Ec2InstanceId, clusterName=None):
    thedetails = {}
    thedetails['Ec2InstanceId'] = Ec2InstanceId
    try:
        if clusterName:
            clusterListResp = ecsClient.list_container_instances(cluster=clusterName)
            instanceListResp = ecsClient.describe_container_instances(cluster=clusterName,containerInstances=clusterListResp['containerInstanceArns'])
            if instanceListResp['containerInstances']:
                for instid in instanceListResp['containerInstances']:
                    logger.debug("Does instance %s match %s",instid['ec2InstanceId'], Ec2InstanceId)
                    if instid['ec2InstanceId'] == Ec2InstanceId:
                        thedetails['clusterName'] = clusterName
                        thedetails['containerInstanceArn'] = instid['containerInstanceArn']
                        logger.info("Cluster name %s, Container instance arn %s",thedetails['clusterName'], thedetails['containerInstanceArn'])
                        return thedetails

        response = ecsClient.list_clusters()
        for cn in response['clusterArns']:
            clusterListResp = ecsClient.list_container_instances(cluster=cn)
            if clusterListResp['containerInstanceArns']:
                instanceListResp = ecsClient.describe_container_instances(cluster=cn,containerInstances=clusterListResp['containerInstanceArns'])
                if instanceListResp['containerInstances']:
                    for instid in instanceListResp['containerInstances']:
                        logger.debug("Does instance %s match %s",instid['ec2InstanceId'], Ec2InstanceId)
                        if instid['ec2InstanceId'] == Ec2InstanceId:
                            thedetails['clusterName'] = cn.split('/')[1]
                            thedetails['containerInstanceArn'] = instid['containerInstanceArn']
                            logger.info("Cluster name %s, Container instance arn %s",thedetails['clusterName'], thedetails['containerInstanceArn'])
                            return thedetails
        ...
        logger.error("Cannot find cluster for instance %s. Error: %s",Ec2InstanceId, e)
    logger.error("Cannot find cluster for instance %s.",Ec2InstanceId)
    return thedetails
...
"""
def checkContainerInstanceTaskStatus(details, containerId=None):
    ...
    if details:
        ...
        if containerId:
            ...
            containerInstancesResp = ecsClient.describe_container_instances(cluster=clusterName,containerInstances=[details['containerInstanceArn'],],)
            for containerInstances in containerInstancesResp['containerInstances']:
...
def lambda_handler(event, context):
    ...
    if 'LifecycleTransition' in message.keys():
        ...
        if message['LifecycleTransition'].find('autoscaling:EC2_INSTANCE_TERMINATING') > -1:
            ...
            if 'containerInstanceId' in message.keys():
                details['containerInstanceId'] = message['containerInstanceId']
                details['Ec2InstanceId'] = Ec2InstanceId
            elif 'NotificationMetadata' in message.keys():
                cluster = json.loads(message['NotificationMetadata'])
                if cluster['clusterName']:
                    details['clusterName'] = clusterName = cluster['clusterName']

            if len(details) != 3:
                details = clusterAndContainerInstanceByInstanceId(Ec2InstanceId, clusterName=clusterName)

            tasksRunning, tmpMsgAppend = checkContainerInstanceTaskStatus(details)
